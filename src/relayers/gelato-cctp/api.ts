import { GELATO_API, CIRCLE_API } from "./constants";
...
export const getRelayTaskStatus = async (taskId: string): Promise<IRelayTaskStatus | null> => {
  try {
    const { task } = (await ky.get(`${GELATO_API}/tasks/status/${taskId}`).json()) as IRelayTaskStatusResponse;
    return task;
  } catch (e) {
...
export const getAttestation = async (messageHash: string): Promise<string | null> => {
  try {
    const { status, attestation } = (await ky.get(`${CIRCLE_API}/attestations/${messageHash}`).json()) as IAttestation;
...
export const postCallWithSyncFee = async (request: CallWithSyncFeeRequest): Promise<string | null> => {
  try {
